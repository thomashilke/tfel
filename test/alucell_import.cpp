...
void test_restriction_extension() {
  ...
  using fe_type = finite_element::edge_lagrange_p1_bubble;
...
void test_alucell_import() {
  try {
    ...
    cfes_type::element velocity(importer::alucell::variable<cfe_type>("/usr/scratch/master/alu-data/trunk/AP32/stat/dbfile_stat",
								      "cuveb", "vitesse", velocity_fes));
    ...
    exporter::ensight6(
      "cuveb",
      velocity.get_component<0>(), "v0",
      velocity.get_component<1>(), "v1",
      velocity.get_component<2>(), "v2");
  
    exporter::ensight6("electrolyte",
      electrolyte_velocity.get_component<0>(), "v0",
      electrolyte_velocity.get_component<1>(), "v1",
      electrolyte_velocity.get_component<2>(), "v2");
